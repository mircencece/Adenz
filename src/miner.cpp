...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("DNZMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("DNZMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("DNZMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dnz-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("DNZMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running DNZMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadDNZMiner() exception");
    } catch (...) {
        LogPrintf("ThreadDNZMiner() exception");
    }

    LogPrintf("ThreadDNZMiner exiting\n");
}
